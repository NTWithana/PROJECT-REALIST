...
"""
            try:
                deep_raw = await safe_model_call(
                    deepseek_reasoner, deep_prompt, timeout=MODEL_TIMEOUT_DEEP, retries=0
                )
                ...
                if parsed and isinstance(parsed, dict) and parsed.get("solution"):
                    ...
                    logger.warning(
                        "Deep model returned invalid JSON or missing solution; using fallback core"
                    )
                    core = fallback_core(
    ...
    try:
        ...
        refined_raw = await safe_model_call(
            gpt5_nano, refine_prompt, timeout=MODEL_TIMEOUT_FAST, retries=0
        )
    ...
    try:
        signal_raw = await safe_model_call(
            gpt5_nano,
    ...
    try:
        if any(k in (cleaned or "").lower() for k in ["class", "function", "api", "repo"]):
            code_raw = await safe_model_call(
                gpt5_nano,
                f"Extract code structure JSON:\n{cleaned}",
                timeout=MODEL_TIMEOUT_FAST,
                retries=0,
            )
